export const articleDomains = ["科技", "教育", "旅游", "情感", "社会", "汽车", "其他"] as const;
...
export const domainConfigs: Record<ArticleDomain, DomainConfig> = {
  科技: {
    ...
    template: "科技蓝",
    colorScheme: "默认蓝",
    aliases: ["ai", "人工智能", "科技", "互联网", "产品", "数码", "大模型", "开源", "芯片"],
  ...
  旅游: {
    ...
    template: "极简白",
    colorScheme: "科技绿",
    aliases: ["旅游", "旅行", "酒店", "民宿", "景点", "出行", "攻略", "路线"],
  ...
  社会: {
    ...
    template: "暖色调",
    colorScheme: "商务橙",
    aliases: ["社会", "社会热点", "民生", "新闻", "事件", "观察", "调查", "舆论", "搞笑"],
  ...
  汽车: {
    ...
    template: "商务灰",
    colorScheme: "默认蓝",
    aliases: ["汽车", "新能源", "车型", "suv", "轿车", "试驾", "评测", "配置", "续航"],
...
> = {
  科技: {
    strong: ["ai", "gpt", "openai", "claude", "agent", "大模型", "人工智能", "芯片", "算力", "开源", "模型", "机器人"],
    medium: ["科技", "互联网", "数码", "手机", "电脑", "应用", "软件", "系统", "云计算", "产品", "发布会", "苹果", "华为", "小米"],
    weak: ["平台", "工具", "公司", "效率", "升级", "版本"],
    patterns: [/(发布|上线|更新|首发|开源).{0,8}(模型|系统|应用|芯片|平台)/i],
  },
  教育: {
  ...
  旅游: {
    strong: ["旅游", "旅行", "景点", "机票", "酒店", "民宿", "签证", "攻略", "路线", "自驾", "露营"],
    medium: ["出行", "高铁", "航班", "目的地", "古镇", "海边", "海岛", "打卡", "避坑"],
    weak: ["门票", "住宿", "行程", "游玩"],
    patterns: [/(去哪玩|值得去|旅行攻略|出行提醒|目的地)/i],
  },
  情感: {
  ...
  社会: {
    strong: ["社会", "民生", "新闻", "事件", "调查", "警方", "通报", "曝光", "舆论", "事故", "现场", "维权", "治理", "案件", "求职", "租房", "消费", "退款", "平台"],
    medium: ["女子", "男子", "老人", "孩子", "家长", "学校", "医院", "司机", "乘客", "网友", "社区", "物业", "商家", "客服", "打工人", "上班", "招聘"],
    weak: ["关注", "热议", "讨论", "提醒", "争议", "画面", "经历", "背后", "回应"],
    patterns: [/(通报|回应|曝光|调查|事件|事故|警方|民生|热议|争议|退款|维权|求职|租房|消费者)/i],
  },
  汽车: {
    strong: ["汽车", "新能源", "suv", "轿车", "试驾", "评测", "续航", "揽巡", "比亚迪", "特斯拉", "理想", "蔚来", "小鹏", "大众", "奔驰", "宝马", "奥迪"],
    medium: ["车型", "新车", "上市", "油耗", "电耗", "智能驾驶", "充电", "底盘", "配置", "驾驶"],
    weak: ["百公里", "马力", "扭矩", "四驱", "增程"],
    patterns: [/(上市|试驾|评测|续航|新车).{0,8}(汽车|suv|轿车|车型)/i],
  },
  其他: {
    strong: ["杂谈", "综合", "其他", "其它"],
    medium: [],
...
export function detectArticleDomain(title: string, tags: string[] = [], source = "", summary = ""): ArticleDomain {
  ...
  if (best.domain !== "其他" && best.score >= 6 && (best.score - secondBestScore >= 2 || best.score >= 10)) {
    return best.domain;
  }
  ...
  if (/微博|抖音|头条|百度|知乎/.test(source)) {
    if (/(高考|考研|中考|老师|学生|课程|备考|学习)/.test(text)) return "教育";
    if (/(汽车|新车|上市|试驾|比亚迪|特斯拉|理想|蔚来|小鹏)/.test(text)) return "汽车";
    if (/(恋爱|婚姻|离婚|分手|家庭|夫妻|伴侣|情绪)/.test(text)) return "情感";
    if (/(事件|通报|警方|曝光|调查|热议|案件|求职|租房|消费|平台|退款)/.test(text)) return "社会";
  }
